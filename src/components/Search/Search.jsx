...
class Search extends React.Component {
  ...
  blur() {
    ...
    setTimeout(() => this.clearResults(), 500);
  }
  clearResults() {
    if (this.mounted) {
      this.setState({ results: null });
  ...
  change(event) {
    ...
    if (input.length >= this.props.minLength) {
      this.setState({ input }, this.search);
    }
    else {
      this.setState({
        input,
  ...
  searchResults(results) {
    // old name - need to fix calling code before removing
  ...
  render() {
    ...
    return (
      <div className={cname} onKeyDown={this.keypress}>
        <input
