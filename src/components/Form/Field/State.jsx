...
import { Text, TextArea, Checkbox, Radio, Select, Static, Search } from '../Input'
...
class State extends React.Component {
    ...
    constructor(props) {
        ...
        if (this.props.validate) {
          ...
          this.validateValue = (value, field) => {
            return new Promise(
              (pass, fail) => this.props.validator(value, field, pass, fail)
            )
          }
        }
        this.handlers = {
        ...
        this.debug(
            ...
            this.props.name, this.props.initialValue, this.props.default, this.state.value
        );
    ...
    onChangeValue(value) {
        const {
            validateOnChange, validateOnInvalid,
            minValidateLength, prepareValue, onChange
        } = this.props;
    ...
    onBlur() {
        ...
        this.setState(
            ...
            () => {
                ...
                if (validateOnBlur) {
                    this.debug('onBlur() - calling validateField() (validateOnBlur option set)');
                    this.validateField();
                }
            }
        );
    ...
    validateField(options) {
        ...
        return new Promise(this.fieldValidator(options))
            .then(
                field => this.setValid(field, this.props.onValid)
            )
            .catch(
                field => {
            ...
            if (empty) {
                ...
                .catch(
                    err => {
                        ...
                        field.message = isString(err)
                          ? err
                          : err.message;
                        this.debug('validation FAIL, message: [%s] => ', field.message, field);
                        reject(field);
                    }
                )
            }
    ...
    submitField(formHandler) {
        if (this.state.validated) {
            let field = { name: this.props.name, label: this.props.label, ...this.state };
            this.debug('submitField() - field is already validated: %s', field.status);
    ...
    renderProps() {
        ...
        const Input = props.Input || inputTypes[props.input] || this.error(
            `Invalid input specified for ${name} field: ${props.input}`
        );
        const field = {
            ...props,
            ...this.handlers,
