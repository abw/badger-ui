...
Add the `badger-ui` library to your project using your favourite package
manager.
...
The `badger-ui` library uses a number of icons from the
[Fontawesome](https://fontawesome.com/) collection.  It has a custom
library built using [svg-icon-librarian](https://www.npmjs.com/package/@abw/svg-icon-librarian).

You will almost certainly want to add further icons to this
collection, either incorporating addition Fontawesome icons
or custom SVG icons.  See the `README` file
in the `icons` directory for further information.
...
Note that the `clean` step happens automatically when you run `pnpm build`.
