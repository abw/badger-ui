...
export const Reset = () => {
  ...
  return <>
    <p>
      This example shows the user of the <code className="code">onLoad</code>{' '}
      handler to store a reference to the context object.  Then we can
      call the <code className="code">reset</code> method to reset the search.
    </p>
    <Search
      placeholder="Animal search"
      className="rounded"
      ...
      // what to do when the result is selected
      onSelect={result => select(result)}
      // get a reference to the search component
      onLoad={setSearch}
    />
    <div className="pad-t-2" style={{marginBottom: '10em'}}>
      { selected &&
        <div className="flex space">
          ...
          <Button text="Reset" onClick={() => search?.reset()}/>
        </div>
      }
    </div>
  </>
